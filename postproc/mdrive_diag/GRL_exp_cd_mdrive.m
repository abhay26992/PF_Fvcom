...
load('./mdrive/melt_qsc.mat')
load('./mdrive/melt_qsd.mat')

load('./mdrive/Tdrive_std.mat')
load('./mdrive/Tdrive_qsc.mat')
load('./mdrive/Tdrive_qsd.mat')

load('./mdrive/ustar_std.mat')
load('./mdrive/ustar_qsc.mat')
load('./mdrive/ustar_qsd.mat')


load C:/PhD/FVCOM/Matlab_Repository/Petermann_Bathy/Bathymetry_IceBridge/Input_Files/Natural_Sill/IBN_M.mat

zz=find(Mobj.zisf==0);
ustar_std(zz,:)=NaN;ustar_qsc(zz,:)=NaN;ustar_qsd(zz,:)=NaN;
Tdrive_std(zz,:)=NaN;Tdrive_qsc(zz,:)=NaN;Tdrive_qsd(zz,:)=NaN;
melt_std(zz,:)=NaN;melt_qsc(zz,:)=NaN;melt_qsd(zz,:)=NaN;
...
for r = 1:cc
    ...
    switch r             
        ...
        case 2
            ...
            run = 'qsc';
            mm(:,r) = vmean(melt_qsc(nMobj.in_sub,:),2)*secpera; % annual mean
            mas(:,r) = vmean(melt_qsc(nMobj.in_sub,is),2)*secpera-mm(:,r); % summer anomaly
            ms(:,r) = vmean(melt_qsc(nMobj.in_sub,is),2)*secpera;
            maw(:,r) = vmean(melt_qsc(nMobj.in_sub,iw),2)*secpera-mm(:,r); % win anomaly
            mw(:,r) = vmean(melt_qsc(nMobj.in_sub,iw),2)*secpera;

            % seasonal anomalies in thermal driving and friction velocity
            tdm(:,r) = vmean(Tdrive_qsc(nMobj.in_sub,:),2);
            tdas(:,r) = vmean(Tdrive_qsc(nMobj.in_sub,is),2)-tdm(:,r);
            tds(:,r) = vmean(Tdrive_qsc(nMobj.in_sub,is),2);
            tdaw(:,r) = vmean(Tdrive_qsc(nMobj.in_sub,iw),2)-tdm(:,r);
            tdw(:,r) = vmean(Tdrive_qsc(nMobj.in_sub,iw),2);

            usm(:,r) = vmean(ustar_qsc(nMobj.in_sub,:),2);
            usas(:,r) = vmean(ustar_qsc(nMobj.in_sub,is),2)-usm(:,r);
            uss(:,r) = vmean(ustar_qsc(nMobj.in_sub,is),2);
            usaw(:,r) = vmean(ustar_qsc(nMobj.in_sub,iw),2)-usm(:,r);
            usw(:,r) = vmean(ustar_qsc(nMobj.in_sub,iw),2);
            
        case 3
            
            run= 'qsd';
            mm(:,r) = vmean(melt_qsd(nMobj.in_sub,:),2)*secpera; % annual mean
            mas(:,r) = vmean(melt_qsd(nMobj.in_sub,is),2)*secpera-mm(:,r); % summer anomaly
            ms(:,r) = vmean(melt_qsd(nMobj.in_sub,is),2)*secpera;
            maw(:,r) = vmean(melt_qsd(nMobj.in_sub,iw),2)*secpera-mm(:,r); % win anomaly
            mw(:,r) = vmean(melt_qsd(nMobj.in_sub,iw),2)*secpera;

            % seasonal anomalies in thermal driving and friction velocity
            tdm(:,r) = vmean(Tdrive_qsd(nMobj.in_sub,:),2);
            tdas(:,r) = vmean(Tdrive_qsd(nMobj.in_sub,is),2)-tdm(:,r);
            tds(:,r) = vmean(Tdrive_qsd(nMobj.in_sub,is),2);
            tdaw(:,r) = vmean(Tdrive_qsd(nMobj.in_sub,iw),2)-tdm(:,r);
            tdw(:,r) = vmean(Tdrive_qsd(nMobj.in_sub,iw),2);

            usm(:,r) = vmean(ustar_qsd(nMobj.in_sub,:),2);
            usas(:,r) = vmean(ustar_qsd(nMobj.in_sub,is),2)-usm(:,r);
            uss(:,r) = vmean(ustar_qsd(nMobj.in_sub,is),2);
            usaw(:,r) = vmean(ustar_qsd(nMobj.in_sub,iw),2)-usm(:,r);
            usw(:,r) = vmean(ustar_qsd(nMobj.in_sub,iw),2);            
...
figure(39);clf
subplot(3,3,1)
plot(nMobj.zisf,[mm(:,2)-mm(:,1),mm(:,3)-mm(:,1)],'.');grid on;ylim([-50 200]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',14,'FontWeight','Bold', 'LineWidth', 2);
xlabel('Ice Draft [m]');ylabel('Melt rate [m/yr]');
title('Annual');

subplot(3,3,2)
plot(nMobj.zisf,[ms(:,2)-ms(:,1),ms(:,3)-ms(:,1)],'.');grid on;ylim([-50 200]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',14,'FontWeight','Bold', 'LineWidth', 2);
%xlabel('Ice Draft [m]');ylabel('Melt rate [m/yr]');
title('Summer');

subplot(3,3,3)
plot(nMobj.zisf,[mw(:,2)-mw(:,1),mw(:,3)-mw(:,1)],'.');grid on;ylim([-50 200]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',14,'FontWeight','Bold', 'LineWidth', 2);
%xlabel('Ice Draft [m]');ylabel('Melt rate [m/yr]');
%[objs, objh] = legend({'qsa-std','qsb-qsa'});
%objhl = findobj(objh, 'type', 'line');
...
subplot(3,3,4)
plot(nMobj.zisf,[tdm(:,2)-tdm(:,1),tdm(:,3)-tdm(:,1)],'.');grid on;ylim([-0.2 0.45]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',14,'FontWeight','Bold', 'LineWidth', 2);
xlabel('Ice Draft [m]');ylabel('\DeltaT [^{o}C]');

subplot(3,3,5)
plot(nMobj.zisf,[tds(:,2)-tds(:,1),tds(:,3)-tds(:,1)],'.');grid on;ylim([-0.2 0.45]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',14,'FontWeight','Bold', 'LineWidth', 2);
%xlabel('Ice Draft [m]');ylabel('\DeltaT [^{o}C]');

subplot(3,3,6)
plot(nMobj.zisf,[tdw(:,2)-tdw(:,1),tdw(:,3)-tdw(:,1)],'.');grid on;ylim([-0.2 0.45]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',14,'FontWeight','Bold', 'LineWidth', 2);
%xlabel('Ice Draft [m]');ylabel('\DeltaT [^{o}C]');
%[objs, objh] = legend({'qsa-std','qsb-qsa'});
%objhl = findobj(objh, 'type', 'line');
...
subplot(3,3,7)
plot(nMobj.zisf,[usm(:,2)-usm(:,1),usm(:,3)-usm(:,1)],'.');grid on;ylim(10^-3*[-3.5 35]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',28,'FontWeight','Bold', 'LineWidth', 2);
xlabel('Ice Draft [m]');ylabel('u* [m/s]');

subplot(3,3,8)
plot(nMobj.zisf,[uss(:,2)-uss(:,1),uss(:,3)-uss(:,1)],'.');grid on;ylim(10^-3*[-3.5 35]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',28,'FontWeight','Bold', 'LineWidth', 2);
%xlabel('Ice Draft [m]');ylabel('u* [m/s]');

subplot(3,3,9)
plot(nMobj.zisf,[usw(:,2)-usw(:,1),usw(:,3)-usw(:,1)],'.');grid on;ylim(10^-3*[-3.5 35]);
set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',28,'FontWeight','Bold', 'LineWidth', 2);
%xlabel('Ice Draft [m]');ylabel('u* [m/s]');
%[objs, objh] = legend({'qsa-std','qsb-qsa'});
%objhl = findobj(objh, 'type', 'line');
