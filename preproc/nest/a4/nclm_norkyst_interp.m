...
for i=1
  for j=1:size(month,1) %To allow the code to scan through the '3' AOTIM months
        ...
        for k=1:nf   % Scan through all days of month, one day at a time
            ...
            %salt
            salt=ncread(romsfile,'salt',[idr(1) jdr(1) 1 1],[irl jrl Inf Inf]); %Change -> Using "rind" to crop
            salt=salt(2:end-1,2:end-1,:,:);
            
            %temp
            temp=ncread(romsfile,'temp',[idr(1) jdr(1) 1 1],[irl jrl Inf Inf]);
            temp=temp(2:end-1,2:end-1,:,:); % 2:end
            temp_r=temp;
            
            %u
            u=ncread(romsfile,'u',[idu(1) jdu(1) 1 1],[iul jul Inf 1]);
            u=0.5*(u(1:end-1,:,:,:)+u(2:end,:,:,:));
            u=u(:,2:end-1,:,:);

            %ubar
            ubar=ncread(romsfile,'ubar',[idu(1) jdu(1) 1],[iul jul 1]);
            ubar=0.5*(ubar(1:end-1,:,:)+ubar(2:end,:,:));
            ubar=ubar(:,2:end-1,:);

            %v
            v=ncread(romsfile,'v',[idv(1) jdv(1) 1 1],[ivl jvl Inf 1]);
            v=0.5*(v(:,1:end-1,:,:)+v(:,2:end,:,:));
            v=v(2:end-1,:,:,:);

            %vbar
            vbar=ncread(romsfile,'vbar',[idv(1) jdv(1) 1],[ivl jvl 1]);
            vbar=0.5*(vbar(:,1:end-1,:)+vbar(:,2:end,:));
            vbar=vbar(2:end-1,:,:);

            %lon
            lon_rho=ncread(romsfile,'lon_rho',[idr(1) jdr(1)],[irl jrl]);
            lon_rho=lon_rho(2:end-1,2:end-1); % 2:end
            
            %lat
            lat_rho=ncread(romsfile,'lat_rho',[idr(1) jdr(1)],[irl jrl]);
            lat_rho=lat_rho(2:end-1,2:end-1); % 2:end
            
            %%% Plots:
            keyboard
            figure(1);clf;pcolor(temp_r(:,:,35));
            figure(2);clf;pcolor(u(:,:,35));
            ...
            for n=1:nt  %each time interval
                ...
                [UBARGEO(:,n),VBARGEO(:,n)]=vel_xy2geo(UBAR(:,n),VBAR(:,n),ANGLE); %Using ANGLE (614 x 1), instead using angle (324x300)?
                ...
                for l=1:nz %for all the depths from 35 (surface) to 1 (bottom)
                    ...
                    T0=squeeze(temp(:,:,l,n));
                    T0=T0(ocn_r);  %At the surface (loop count=1) : Check for each day (nc file -> 1,2,20), where the boundary points are (TT) and what values they have (TT). 
            ...
            for n=1:nn %One node point (and all depth levels) at a time
                ...
                for l=1:nt
                    ...
                    SS=squeeze(SALT(n,:,l));  
                    TT=squeeze(TEMP(n,:,l)); % node n = 1:618, all depth, l=1 (always)
                    
                    salt(n,:,l)=sum(coef.*SS(ind),2);
                    temp(n,:,l)=sum(coef.*TT(ind),2); %Filling up temp one node point (and all depth levels) at a time
                end
            end
        ...
        save(['C:/PhD/FVCOM/Matlab_Repository/Petermann_Bathy/Setup_Nesting/Roms_a4_daily_nest' filename],'nclm','-v7.3')
