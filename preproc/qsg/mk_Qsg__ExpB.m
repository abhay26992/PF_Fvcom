...
% Then,distribute the runoff evenly over only the channel cells. So, first
% identify which of the 90 'glc_bcell' corresponds to the PGIS channels.
% Executed in Exp_B_Design_Plan.m script.

load ./rc_expb; %9 chosen (Exp_B_Design.m) channel river cells 

Q_sg=552; %Summer mean value in m^3/s, for now, taken from Mankoff_BMB

river.pnid=(rc_expb)'; %Cell numbers of river mouth at the GL 
river.riverNames=num2str((1:numel(rc_expb))'); %Corresponding river name

transport = repmat(Q_sg/numel(rc_expb),[numel(rc_expb),1]); %Equally distributed over all cells along the GL
...
pval_hc=gsw_p_from_z(-hc(rc_expb),latc(rc_expb)); %pressure in dbar from h
temp=gsw_t_freezing(zeros(numel(rc_expb),1),pval_hc); %pressure dependent freezing point of freshwater

salt=zeros(numel(rc_expb),1); %salt =0
...
fname='riverNamelist_expB.nml';
fid=fopen(fname,'w+');

riverFile='riverdata_expB.nc';
